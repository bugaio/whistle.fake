...
function handleResponse(promise, deferred) {
...
function httpGet(options) {
  return setType(function() {
    ...
    handleResponse(request(formatOptions(options, 'GET')), deferred);
    return deferred.promise;
  }, 'get');
}
...
function httpPost(options) {
  return setType(function() {
    var deferred = Q.defer();
    removeAcceptEncoding(options);
    handleResponse(request(formatOptions(options, 'POST')), deferred);
    return deferred.promise;
  }, 'post');
}
...
function httpRequest(options) {
  return setType(function() {
    var deferred = Q.defer();
    removeAcceptEncoding(options);
    handleResponse(request(formatOptions(options)), deferred);
    return deferred.promise;
  }, 'request');
}
...
function loadModule(modulePath) {
  if (modulePath && /^\w/.test(modulePath)) {
    return require(path.join(HOME_DIR, 'whistle.fake/node_modules', modulePath));
  }
